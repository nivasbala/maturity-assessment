...
export default function ResearchingPage() {
  ...
  // Poll /research-summary until is_ready=true, then navigate to pillars
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (data.is_ready) {
          ...
          setProgress(100)
          // Brief pause so the bar completes visually before transition
          setTimeout(() => {
            if (!cancelled) navigate(`/assess/${token}/pillars`)
          }, 600)
        } else {
          // Back off: 2s → 4s → 6s → cap at 8s
