      ...
      <div className="flex h-[calc(100vh-56px)]">
        ...
        <div className="w-96 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-col">
          <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <div>
              <a href="/admin/pillars" className="text-xs text-[#4F46E5] hover:underline">← Pillars</a>
              <h2 className="text-sm font-bold text-[#1B2B4B] dark:text-gray-100 mt-0.5">Questions</h2>
            </div>
            <button
              onClick={startCreate}
              className="text-xs bg-[#4F46E5] text-white px-3 py-1.5 rounded-lg font-medium hover:bg-indigo-700"
            >
              + New
            </button>
          </div>
          ...
          <div className="overflow-y-auto flex-1">
            ...
            {questions.map((q) => (
              <button
                key={q.id}
                onClick={() => startEdit(q)}
                className={`w-full text-left px-4 py-3 border-b border-gray-100 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors ${selected?.id === q.id ? 'bg-blue-50 dark:bg-blue-900/20 border-l-2 border-l-[#4F46E5]' : ''}`}
              >
                    ...
                    {form.answer_options.map((opt, i) => (
                      <div key={opt.maturity_level} className="flex items-start gap-2">
                        ...
                        <textarea
                          ...
                          rows={2}
                          className="flex-1 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-[#4F46E5]"
                        />
                      </div>
                    ))}
                  </div>
