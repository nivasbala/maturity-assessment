...
async def compute_pillar_score(
    ...
    Returns:
        Tuple of (pillar_score, maturity_level, maturity_label).
    """
    if not answer_pairs:
    ...
    for question_id, answer_option_id in answer_pairs:
        ...
        numerator += ao.maturity_level * q_weight * persona_weight
        denominator += 4 * q_weight * persona_weight
    ...
    logger.info(
        "compute_pillar_score: persona=%s score=%.2f level=%d label=%s",
        persona,
        score,
        level,
        label,
