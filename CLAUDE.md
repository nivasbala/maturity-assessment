...
- Agent 1 fires at **prospect registration** (non-blocking) — it needs prospect-provided context, which is not available until registration; a hash of the six research inputs (stored inside research_cache) plus a 3-day TTL skip re-running Agent 1 when nothing has changed
