...
-- Note: the "Registration Context" tab on both report pages (05-architecture-api.md Section 3.3/3.4)
-- reads infrastructure_location, tech_stack_description, current_tools, and key_challenges_input
-- live from the `prospects` row (not a point-in-time snapshot); prospect_additional_notes on that
-- same tab reads from the `assessments` row, reflecting the copy made at confirm-research time.
CREATE TABLE reports (
    ...
    research_data    JSONB,                             -- prospect's research cache at submission time; shown on the Research Summary tab of both the prospect-facing report and the internal Report Detail page
    created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
